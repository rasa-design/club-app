      ...
      {isAdmin && adding && (
        <div className="rounded-xl border p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="flex-1" onClick={() => { setAdding(false); setSize(''); setError('') }}>
              キャンセル
            </Button>
            <Button size="sm" className="flex-1" onClick={add} disabled={saving || !size.trim()}>
              {saving ? '追加中...' : '追加する'}
            </Button>
          </div>
        </div>
